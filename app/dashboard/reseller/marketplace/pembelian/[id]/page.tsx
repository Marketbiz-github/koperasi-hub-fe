...
export default function ResellerPurchasesDetailPage() {
    ...
    const handlePay = async () => {
        ...
        if (order.payment_category === 'piutang') {
            ...
            try {
                const token = await getAccessToken();
                let targetId = debt?.id || order.debt?.id;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Detail Pembelian Reseller</h1>
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        <div className="relative w-10 h-10 rounded border overflow-hidden shrink-0">
                                                            {item.product?.image || item.product_image ? (
                                                                <Image src={item.product?.image || item.product_image} alt="P" fill className="object-cover" />
                                                            ) : (
                                                                <div className="bg-gray-100 w-full h-full flex items-center justify-center text-[10px] text-gray-400">No Img</div>
                                                            )}
                                                        </div>
                                                    ...
                                                    {settlementStatus === 'LUNAS' && (
                                                        <Button
                                                            ...
                                                            onClick={() => {
                                                                ...
                                                                if (pid) {
                                                                    window.location.href = `/dashboard/reseller/produk/tambah?duplicate=${pid}`;
                                                                } else {
                        ...
                        <CardContent className="pt-4 space-y-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-xs text-gray-600 leading-relaxed">
                                    {order.customer_address || order.shipping_address?.address}, {order.customer_subdistrict || order.shipping_address?.subdistrict && `${order.customer_subdistrict || order.shipping_address?.subdistrict}, `}{order.customer_district || order.shipping_address?.district}, {order.customer_city || order.shipping_address?.city}, {order.customer_province || order.shipping_address?.province} {order.customer_zipcode || order.shipping_address?.zipcode}
                                </p>
                            </div>
